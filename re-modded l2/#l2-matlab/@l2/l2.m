...
%         parameters;     % default parameters.l2
    ...
    methods
        ...
        function result = simulate(this, t_max, varargin)
            ...
            %   default parameters for 10 timesteps and without the arduino
            ...
            %   In case the second argument contains the string 'COM' the
            %   meaning of all arguments is shifted by one place. This
            %   means that an arduino is used in running the model and that
            %   it is connected to the COM port that is specified in the
            %   second argument. The third argument indicates the scenario
            %   that is to be used and the fourth argument indicates the
            %   parameter set to be used. The third and fourth arguments
            %   remain optional.
            ...
            if nargin>=5
                paramToSet = varargin{3};
                useArduino = true;
            end

            % initialise trace, parameters and arduino
            ...
            if useArduino
%-------------------------------------------------------------------------
                %this.controller = arduino(comPort, 'Uno');
                this.controller = arduino('/dev/tty.usbmodem411','Uno');
                disp('my arduino')
                % readVoltage(this.controller,'A0')
%-------------------------------------------------------------------------
            end
